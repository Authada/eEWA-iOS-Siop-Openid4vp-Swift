...
final class SiopOpenID4VPTests: DiXCTest {
  ...
  func testRequestObjectGivenValidJWT() async throws {
    ...
    //This test fails since validation for vprequest is used
    //passByValueJWT does not include "typ" in JWS header
    //to make this test run again update passByValueJWT with compatible JWS header
    
    try XCTSkipIf(true, "update TestsConstants.passByValueJWT data to make this test work again")
    ...
    guard let validatedAuthorizationRequestData else {
      return
    }
